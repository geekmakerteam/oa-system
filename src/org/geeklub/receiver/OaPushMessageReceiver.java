...
import org.geeklub.vass.activitys.MainActivity;
...
public class OaPushMessageReceiver extends FrontiaPushMessageReceiver {
	/** TAG to Log */
	public static final String TAG = OaPushMessageReceiver.class.getSimpleName();
	private static final String tag = "MyOaPushMessageReceiver";
	...
	public void onBind(Context context, int errorCode, String appid, 
		...
		if (errorCode == 0) {
			Log.i(tag, "绑定成功");
			//			设置绑定已经绑定
			OaPushUtil.setBind(context, true);

			//将服务器返回的userID和channelID保存到本地
			OaPushUtil.setChannelID(context, channelId);
			OaPushUtil.setUserID(context, userId);


			//			提交UserID和channelID到服务器
			RequestParams requestParams = new RequestParams();
			requestParams.put(Values.USER_ID,OaPushUtil.getUserID(context));
			requestParams.put(Values.CHANNEL_ID,OaPushUtil.getChannelID(context));
			Log.i(tag, "requestparams==="+requestParams);

			Log.i(tag, "开始发送userid和channelID");

			OaAsyncHttpClient.post(Url.PUSH_INIT, requestParams, new AsyncHttpResponseHandler(){
				@Override
	...
	public void onUnbind(Context context, int errorCode, String requestId) {
		...
		// 解绑定成功，设置未绑定flag，
		if (errorCode == 0) {
			OaPushUtil.setBind(context, false);
		}
		// Demo更新界面展示代码，应用请在这里加入自己的处理逻辑
		//		updateContent(context, responseString);
	}
